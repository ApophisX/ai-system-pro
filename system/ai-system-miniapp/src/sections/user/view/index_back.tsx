  ...
  const lessorFunctionGroups: FunctionGroupProps[] = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          title: '待处理',
          subtitle: '查看待处理订单',
          badge: lessorStats.pendingOrderCount,
          onClick: () => {
            Taro.navigateTo({
      ...
      items: [
        {
          ...
          onClick: () => {
            Taro.navigateTo({
              url: paths.webview(webPaths.myOrders),
            });
          },
          color: '#10b981',
        },
        {
          ...
          onClick: () => {
            Taro.navigateTo({
              url: paths.webview(`${webPaths.myOrders}?status=created`),
            });
          },
          color: '#ef4444',
        },
  ...
  return (
    <View className="min-h-screen bg-gray-50 flex flex-col safe-area-bottom" style={{ height: '100vh' }}>
      <ScrollView scrollY className="flex-1" style={{ flex: 1 }} enhanced showScrollbar={false} enableBackToTop>
        ...
        <LesseeStatsCards stats={lesseeStats} />
        <LessorStatsCards stats={lessorStats} />

        <View className="px-4 pb-12">
          {lesseeFunctionGroups.map(group => (
            <FunctionGroup key={group.title} {...group} />
          ))}

          {lessorFunctionGroups.map(group => (
            <FunctionGroup key={group.title} {...group} />
          ))}
