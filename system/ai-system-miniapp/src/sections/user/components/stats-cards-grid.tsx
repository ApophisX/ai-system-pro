...
export function StatCardGrid({ icon, title, value, subtitle, color, onClick, valuePrefix }: StatCardProps) {
  return (
    <View
      // className="flex-1 min-w-[45%] p-4 rounded-2xl bg-white shadow-sm border border-gray-100 mb-3"
      className="flex-1 p-4 rounded-2xl bg-white shadow-sm border border-gray-100 mb-3"
    ...
    >
      ...
      <Text className="text-gray-500 text-xs font-medium mt-0.5">{title}</Text>
    </View>
  );
}
...
export function LessorStatsCards({ stats = DEFAULT_LESSOR_STATS }: { stats?: MyApi.OutputLessorStatisticsDto }) {
  ...
  return (
    <View className="px-4">
      {/* <Text className="text-base font-bold text-gray-900 mb-3 px-1">出租数据</Text> */}
      <View className="flex flex-row flex-wrap gap-x-3">
        {cards.map(card => (
          <StatCardGrid key={card.title} {...card} />
        ))}
      </View>
    </View>
  );
...
export function LesseeStatsCards({ stats }: { stats: MyApi.OutputLesseeStatisticsDto }) {
  ...
  return (
    <View className="px-4">
      {/* <Text className="text-base font-bold text-gray-900 mb-3 px-1">租赁数据</Text> */}
      {/* <View className="flex flex-row flex-wrap gap-x-3"> */}
      {/* 一行三列 */}
      <View className="grid grid-cols-3 gap-x-3">
        {cards.map(card => (
          <StatCardGrid key={card.title} {...card} />
        ))}
      </View>
