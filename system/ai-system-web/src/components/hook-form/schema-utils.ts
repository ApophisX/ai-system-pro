...
export const schemaUtils = {
   ...
   */
  /**
   * 校验中国大陆手机号（严格规则：以1开头，第二位3-9，后面9位数字）
   */
  phoneNumber: (props?: { error?: SchemaErrorMessages; isValid?: (val: string) => boolean }) =>
    z
      .string()
      .min(1, { error: props?.error?.required ?? '请输入手机号！' })
      .refine(
        (val) => {
          if (props?.isValid) {
            return props.isValid(val);
          }
          // 中国大陆手机号正则：以1开头，第二位3-9，长度11位
          return /^1[3-9]\d{9}$/.test(val);
        },
        {
          error: props?.error?.invalid ?? '请输入有效的手机号！',
        }
      ),
   ...
   */
  date: (props?: { error?: SchemaErrorMessages; isRequired?: boolean }) =>
    z.preprocess(
      (val) => (val === undefined ? null : val), // 在验证前预处理输入值
      z.union([z.string(), z.number(), z.date(), z.null()]).check((ctx) => {
        const value = ctx.value;

        if (props?.isRequired && (value === null || value === '')) {
          ctx.issues.push({
            code: 'custom',
            message: props?.error?.required ?? '请输入日期！',
            input: value,
          });
          return;
        }
        ...
        if (!dayjs(value).isValid()) {
          ctx.issues.push({
            code: 'custom',
            message: props?.error?.invalid ?? '请输入有效的日期！',
            input: value,
          });
        }
      })
