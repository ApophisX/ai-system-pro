...
export function UploadAvatar({
  ...
  const avatarUrl = useMemo(() => {
    ...
    return prefixPath + previewUrl;
  }, [prefixPath, previewUrl]);
  ...
  const renderPreview = () =>
    hasSelectedFile && previewUrl && <PreviewImage alt={fileMeta.name} src={avatarUrl} />;
  ...
  return (
    <UploadWrapper {...slotProps?.wrapper} className={uploadClasses.wrapper}>
      ...
      {showFilesRejected && <RejectedFiles files={fileRejections} maxSize={dropzoneOptions.maxSize}  {...slotProps?.rejectedFiles} />}
    </UploadWrapper>
  );
}
